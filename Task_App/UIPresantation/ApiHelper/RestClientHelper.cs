...
namespace UIPresantation.ApiHelper
{
    public sealed class RestClientHelper : IRestClientHelper
    {
        ...
        public async Task<string> DeleteAsync(string requestUri, string jwt, Dictionary<string, string> additionalHeaders = null)
        {
            string result = "Error";
            using (HttpClientHandler httpClientHandler = new HttpClientHandler())
            {
                using (HttpClient httpClient = new HttpClient(httpClientHandler))
                {
                    ...
                    AddHeaders(httpClient, additionalHeaders);
                    result = await httpClient.DeleteAsync(requestUri).Result.Content.ReadAsStringAsync();
                }
            }
            return result;
        }
        ...
        public async Task<string> PatchAsync<T>(string requestUri, T request, Dictionary<string, string> additionalHeaders = null) where T :
        class
        {
            string result = "Error";
            using (HttpClientHandler httpClientHandler = new HttpClientHandler())
        ...
        private void AddHeaders(HttpClient httpClient, Dictionary<string, string> additionalHeaders)
        {
            ...
            {
                httpClient.DefaultRequestHeaders.Add(current.Key,
                current.Value);
            }
        }
    }
